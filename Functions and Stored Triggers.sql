...
BEGIN
    IF NEW.status = 'Accepted' THEN
		...
		_email = (SELECT email FROM bid WHERE advertisementID = NEW.advertisementID);
		_status = (SELECT status FROM bid WHERE advertisementID = NEW.advertisementID);
		_price = (SELECT price FROM bid WHERE advertisementID = NEW.advertisementID);
		_creationDateTime = (SELECT creation_date_and_time FROM bid WHERE advertisementID = NEW.advertisementID);
	...
	ELSEIF NEW.status = 'Expired' OR NEW.status = 'Rejected' OR NEW.status = 'Retracted' THEN
		_email = (SELECT email FROM bid WHERE advertisementID = NEW.advertisementID);
		_status = (SELECT status FROM bid WHERE advertisementID = NEW.advertisementID);
		_price = (SELECT price FROM bid WHERE advertisementID = NEW.advertisementID);
		_creationDateTime = (SELECT creation_date_and_time FROM bid WHERE advertisementID = NEW.advertisementID);
...
--Procedure to expire bid after ad has been closed
--Bid set to Retracted if advertisement is closed at current date BEFORE the date of pickup
--Bid set to Expired if advertisement is closed at current date AFTER the date of pickup
...
--Function to delete bid for both admin and user
CREATE OR REPLACE FUNCTION deleteBid(_email varchar, _advertisementID bigint) RETURNS void AS $$
BEGIN
	UPDATE bid SET status = 'Expired' WHERE email = _email AND advertisementID = _advertisementID;
END;
$$ LANGUAGE plpgsql;
